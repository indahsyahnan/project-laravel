...
<div class="ml-3" style="margin-left: 15px; margin-right: 15px">
  ...
  <table class="table table-striped">
    ...
    <tbody>
      @foreach($komentarjawab as $key => $komentarjawab)
      <tr>
      	<td>{{$key+1}}</td>
      	<td>{{$komentarjawab->isi}}</td>
      </tr>
      @endforeach
    </tbody>
  </table>
  <center><h4 style="padding-top: 7px; padding-bottom: 7px">Komentar Disini</h4></center>
  <form action="/komentarjawaban/{{$jawab[0]->id}}" method="POST" style="margin-left: 5px; margin-right: 5px">
    @csrf
